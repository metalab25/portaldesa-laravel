<section class="pt-7 pb-4 position-relative overflow-hidden">
    <div class="card tilt" data-tilt>
        <img src="{{ asset('assets/web/img/shapes/waves-white.svg') }}" alt="pattern-lines"
            class="position-absolute top-0 opacity-6">
    </div>
    <div class="container">
        <div class="row">
            <div class="mx-auto bg-gradient-dark border-radius-xl">
                @foreach ($sambutan as $item)
                    <div class="row py-5">
                        <div class="col-xl-3 col-md-6 px-5 position-relative">
                            <div class="border-radius-xl tilt" data-tilt>
                                @if ($item->avatar)
                                    <img src="{{ asset('storage/' . $item->avatar) }}"
                                        class="img border-radius-xl max-width-300 w-100 mt-n8 position-relative z-index-2">
                                @else
                        ...
                        <div class="col-xl-8 col-md-5 z-index-2 position-relative px-md-3 px-5 my-md-auto mt-6">
                            <div>
                                <svg class="text-warning" width="35px" height="35px" viewBox="0 0 270 270"
                                    ...
                                    <g id="quote-down" stroke="none" stroke-width="1" fill="none"
                                        ...
                                        <path
                                            ...
                                            id="quote-up" fill="#dc940d" fill-rule="nonzero"
                            ...
                            <p class="text-lg text-white">
                                {{ $item->excerpt }} <a href="{{ url('details/' . $item->slug) }}"
                                    class="link-sambutan">
                                    selengkapnya</a>
                            </p>
                            <p class="text-white font-weight-bold text-sm font-quicksand">
                                {{ $item->created_at }}<br>
                                {{ $item->jabatan_sambutan }} - <span
                                    class="text-sm font-weight-normal">{{ $item->pemberi_sambutan }}</span></p>
                        </div>
                    </div>
                @endforeach
            </div>
